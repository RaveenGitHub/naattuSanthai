...
def test_mvp_plan_page_and_home_link_are_removed():
    home = client.get("/home")
    assert home.status_code == 200
    assert "/mvp-plan" not in home.text
    assert client.get("/mvp-plan").status_code == 404
...
def test_authenticated_farmer_profile_sets_weather_and_market_defaults():
    ...
    soil_response = isolated_client.get("/soil-health", follow_redirects=False)
